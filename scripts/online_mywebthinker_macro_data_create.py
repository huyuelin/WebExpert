...
INPUT_JSONL_FILE: str = \
    "/workspace2/linwen/dpo_dataset_prepare/train_stage1_all_feat_v3_prompt_v4_0519.jsonl"
# 输出目录及文件（支持断点续跑）
OUTPUT_DIR: str = \
    "/workspace2/linwen/mybanck_webthinker/scripts/macro_data_by_mywebthinker"
...
def safe_get_llm_response(
    ...
    """简化版安全调用：直接调用本地 get_llm_response，出现异常时返回空串。"""
    try:
        return _orig_get_llm_response(
            prompt=prompt,
            stop=stop,
            model=model,
            base_url=base_url,
        ...
        print(f"[safe_get_llm_response] 捕获异常：{e}，返回空串继续流程。")
        return ""
